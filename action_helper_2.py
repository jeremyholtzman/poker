...
def blind_action(blind_order, small_blind, big_blind, cards, balance_dict):
    ...
    if balance_dict["Player {0}".format(blind_order[-1])] > big_blind:
        ...
        all_in_amount = balance_dict["Player {0}".format(blind_order[-1])]
        dollar_dict["Player {0}".format(blind_order[-1])]=all_in_amount
        action_df = action_df.append(pd.DataFrame([["Player {0}".format(blind_order[-1]),'pre-flop','big blind - all in',all_in_amount]], columns = action_df_columns))
    ...
    if balance_dict["Player {0}".format(blind_order[-2])] > small_blind:
        ...
        all_in_amount = balance_dict["Player {0}".format(blind_order[-2])]
        dollar_dict["Player {0}".format(blind_order[-2])]=all_in_amount
        action_df = action_df.append(pd.DataFrame([["Player {0}".format(blind_order[-2]),'pre-flop','small blind - all in',all_in_amount]], columns = action_df_columns))
    ...
    if len(blind_order) > 1:
        ...
        while in_process:
            ...
            while j < 10:
                ...
                if (dollar_dict["Player {0}".format(turn)] < total_to_call_round) & (a == 'check'):
                    j+=1
                    a = 'fold'
...
def post_flop_betting(order, action_df, small_blind, big_blind, round, cards, balance_dict):
    ...
    while in_process == True:
        ...
        while j < 10:
            a = input('what would you like to do?')
            #this line is breaking it. need to get rid of this check and do it another way
